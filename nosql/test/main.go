...
func main() {
	...
	err = mongoClient.InsertMany(DBName, TableName, user7)
	if err != nil {
	...
	//
	//err = mongoClient.FindUseCursor(DBName, TableName, 1, bson.D{}, &list, callback)
	//if err != nil {
	...
	//$in 中不存在的id不抛异常
	//err = mongoClient.DeleteMany(DBName, TableName, bson.D{{"_id", bson.D{{"$in", []int64{4, 3}}}}})
	//if err != nil {
	//	nosql.MongoStdLogger.Print("DeleteOne : ", " error : ", err)
	//}

	err = mongoClient.DeleteMany(DBName, TableName, bson.D{{"_id", bson.D{{"$in", []string{"63bd6d6fcc920900990fa5f3"}}}}})
	if err != nil {
		nosql.MongoStdLogger.Print("DeleteOne : ", " error : ", err)
	}
	//12.创建索引
